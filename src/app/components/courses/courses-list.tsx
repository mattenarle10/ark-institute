    ...
    <section className="py-12 sm:py-16 md:py-20 bg-gray-50">
      <div className="mx-auto max-w-7xl px-6 sm:px-8 md:px-16">
        ...
        <div ref={regHeaderRef} className="mb-6 sm:mb-8">
          ...
          <p data-reveal className="mt-1 text-sm text-gray-600">
            These programs are accredited by TESDA.
          </p>
        </div>
        <div ref={regListRef} className="space-y-8 sm:space-y-10">
          {registeredCourses.map((course, idx) => (
            ...
            >
              ...
              <div
                className={`flex flex-col ${idx % 2 === 1 ? "md:flex-row-reverse" : "md:flex-row"} items-stretch`}
                style={{ opacity: 1 }}
              >
                {/* Image */}
                ...
                <div className="w-full md:w-1/2 p-6 sm:p-8 md:p-10 flex flex-col justify-center">
                  <div className="flex flex-wrap items-center gap-2 text-[11px] sm:text-xs font-semibold">
                    <span className="inline-flex items-center gap-1 rounded-full bg-primary/5 px-2 py-0.5 ring-1 ring-primary/10 text-primary">
                      TESDA Accredited
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 ring-1 ring-emerald-200 text-emerald-700">
                      Registered
                    </span>
                  </div>
                  <h3 className="mt-3 text-2xl sm:text-[28px] md:text-[30px] font-semibold tracking-tight text-gray-900">
                    {course.title}
                  </h3>
                  <p className="mt-3 text-[13.5px] sm:text-base leading-relaxed text-gray-600">
                    {course.description}
                  </p>
                </div>
              </div>
        ...
        <div className="mt-12 sm:mt-14">
          <div ref={csHeaderRef}>
            ...
            <p data-reveal className="mt-1 text-sm text-gray-600">
              These programs are currently on process.
            </p>
          </div>
          <div
            ref={csGridRef}
            className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6"
          >
            {comingSoonCourses.map((course) => (
              ...
              >
                ...
                <div className="relative flex items-start justify-between gap-3">
                  ...
                  <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-accent/5 px-2 py-0.5 text-[11px] sm:text-xs font-semibold ring-1 ring-accent/20 text-accent">
                    On process
                  </span>
                </div>
              </div>
