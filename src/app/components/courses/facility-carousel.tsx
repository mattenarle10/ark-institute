...
export default function FacilityCarousel({ images, facilityId }: FacilityCarouselProps) {
  ...
  useEffect(() => {
    ...
    if (dotsRef.current && prevBtnRef.current && nextBtnRef.current) {
      gsap.fromTo([prevBtnRef.current, nextBtnRef.current],
        { opacity: 0, scale: 0.8 },
  ...
  return (
    <div ref={carouselRef} className="relative w-full h-full overflow-hidden rounded-lg">
      ...
      <div ref={dotsRef} className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 z-10">
        {images.map((_, idx) => (
          <button
